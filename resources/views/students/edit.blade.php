<x-app-layout>
    ...
    <form method="POST" action="{{ route('students.update', $student) }}" class="max-w-3xl space-y-6">
        @csrf
        @method('PUT')

        @if (auth()->user()->isTeacher())
            <div class="bg-white rounded-2xl border border-gray-200 p-6">
                <h2 class="text-base font-semibold text-gray-900 mb-4">Jenis Kelamin</h2>
                <div class="space-y-4">
                    <div>
                    ...
                    <p class="text-sm text-gray-500">Sebagai guru, Anda hanya dapat mengubah jenis kelamin siswa.</p>
                </div>
            </div>
        @else
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <x-text-input id="nis" name="nis" type="text" class="mt-1 block w-full" :value="old('nis', $student->nis)" />
                    <x-input-error :messages="$errors->get('nis')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="nisn" :value="__('NISN')" />
                    <x-text-input id="nisn" name="nisn" type="text" class="mt-1 block w-full" :value="old('nisn', $student->nisn)" />
                ...
                <div>
                    ...
                    <x-text-input id="birth_place" name="birth_place" type="text" class="mt-1 block w-full" :value="old('birth_place', $student->birth_place)" />
                </div>

                <div>
                    <x-input-label for="birth_date" :value="__('Tanggal Lahir')" />
                    <x-text-input id="birth_date" name="birth_date" type="date" class="mt-1 block w-full" :value="old('birth_date', $student->birth_date?->format('Y-m-d'))" />
                    <x-input-error :messages="$errors->get('birth_date')" class="mt-2" />
                </div>

                <div class="sm:col-span-2">
                    <x-input-label for="address" :value="__('Alamat')" />
                    <textarea id="address" name="address" rows="2" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white">{{ old('address', $student->address) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <x-text-input id="enrolled_at" name="enrolled_at" type="date" class="mt-1 block w-full" :value="old('enrolled_at', $student->enrolled_at?->format('Y-m-d'))" />
                </div>
            </div>
        </div>
                ...
                <div>
                    ...
                    <select id="class_id" name="class_id" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white">
                        ...
                        @foreach ($classes as $cls)
                            <option value="{{ $cls->id }}" @selected(old('class_id', $student->class_id) == $cls->id)>{{ $cls->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="major_id" name="major_id" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white">
                        ...
                        @foreach ($majors as $major)
                            <option value="{{ $major->id }}" @selected(old('major_id', $student->major_id) == $major->id)>{{ $major->code }} — {{ $major->name }}</option>
                        @endforeach
                    </select>
                </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <x-text-input id="parent_name" name="parent_name" type="text" class="mt-1 block w-full" :value="old('parent_name', $student->parent_name)" />
                </div>

                <div>
                    <x-input-label for="parent_phone" :value="__('No. HP Orang Tua')" />
                    <x-text-input id="parent_phone" name="parent_phone" type="text" class="mt-1 block w-full" :value="old('parent_phone', $student->parent_phone)" />
                </div>

                <div class="sm:col-span-2">
                    <x-input-label for="parent_email" :value="__('Email Orang Tua')" />
                    <x-text-input id="parent_email" name="parent_email" type="email" class="mt-1 block w-full" :value="old('parent_email', $student->parent_email)" />
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('students.show', $student) }}" class="px-4 py-2 rounded-lg border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-50">Batal</a>
