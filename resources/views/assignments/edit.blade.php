<x-app-layout>
    ...
    <form method="POST" action="{{ route('assignments.update', $assignment) }}" class="max-w-2xl space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6 space-y-4">
            ...
            <div>
                ...
                <x-text-input id="due_at" name="due_at" type="datetime-local" class="mt-1 block w-full" :value="old('due_at', $assignment->due_at?->format('Y-m-d\TH:i'))" />
                <x-input-error :messages="$errors->get('due_at')" class="mt-2" />
            </div>

            <div>
                <x-input-label for="description" :value="__('Deskripsi / Petunjuk')" />
                <textarea id="description" name="description" rows="8" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white" required>{{ old('description', $assignment->description) }}</textarea>
                <x-input-error :messages="$errors->get('description')" class="mt-2" />
            </div>
        </div>

        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('assignments.show', $assignment) }}" class="px-4 py-2 rounded-lg border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-50">Batal</a>
